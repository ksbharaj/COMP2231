...
// Implementation of ArrayHeap.java that helps to be build a heap in place using
...
public class ArrayHeap<T> extends ArrayBinaryTree<T> implements HeapADT<T> 
{
    ...
    // Option to initialise an Array Heap from an exisitng array
     ...
     * adding a node.
     */
    // Operation count is incremented by one for each comparison done
    ...
    private void heapifyRemove()
    {
        ...
        // Operation count is incremented due to comparison done below
        operationCount += 1;
        ...
        {
            // while loop starts with a comparison, so an operation count is incremented
            operationCount += 1;
            tree[node] = tree[next];
            node = next;
            left = 2 * node + 1;
    ...
    // Returns the number of operations to compare with other algorithms
    ...
    {
        T[] printout = Arrays.copyOf(tree, count);;
        return Arrays.toString(printout);
    }
}
